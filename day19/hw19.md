...
[X] I completed this assignment with assistance from Keenan
...
2. ∃y. ∀x. loves(x, y) True, everyone loves C
3. ∀x. ∃y. loves(x, y) True, everyone loves someone
4. ∀y. ∃x. loves(x, y) True, everyone has someone that loves them
5. ∃x. ∃y. loves(x, y) True, because it is a subset of above
6. ∃x. ∀y. ¬loves(x, y) False Nobody doesn't love anyone
7. ∃x. ¬∀y. loves(x, y) True there is someone that does not love everyone (such as A)
...
let y = s(0):
  sum( y, 0, y ) = sum( s(0), 0, s(0) )
then
  sum( y, s(0), s(y) ) = sum( s(0), s(0), s(s(0)) )
...
inductive hypothesis
  there exists k, such that sum( 0, k, k )
...
`max_subarray` is not a recursive function, so this doesn't make sense.

b. Is `max_subarray` an example of divide and conquer? Why or why not?

It is not divide and conquer because it is not recursive.

c. Is `max_subarray` an example of dynamic programming? Why or why not?

It is not an example of dynamic programming because it does use any memoization.
...
This would not help anything because it is not recursive and it runs in O(N)... I considered it.
...
I don't understand the question being asked. Why would you memoize this?
...
This question does not apply to me because I did not do the above question. I do not see any benefit in memoizing it.
